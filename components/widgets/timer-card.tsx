...
export function TimerCard({ sessions, settings, activeProject }: TimerCardProps) {
  ...
  const modeGlow: Record<TimerMode, string> = {
    focus: "border-[rgba(var(--accent),0.24)] shadow-[0_0_0_1px_rgba(var(--accent),0.08),0_28px_80px_rgba(0,0,0,0.18)]",
  };

  return (
    <section className={cn(
      "panel relative overflow-hidden rounded-2xl border p-5 transition-all duration-500 sm:p-6",
      modeGlow[timer.mode]
    )}>
      <div className="relative z-10 flex flex-col items-center">
        <div className="rounded-xl border border-[rgba(var(--line),0.45)] bg-[rgba(var(--bg-secondary),0.55)] px-3.5 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[rgb(var(--muted))]">
          {modeLabels.focus}
        </div>

        <div className="mt-7 text-center sm:mt-8">
          <h2 className="text-[clamp(4.2rem,12vw,7.6rem)] font-black leading-none text-white">
            {formatDuration(displaySec)}
          </h2>

          <div className="surface-card-quiet mx-auto mt-4 inline-flex max-w-[34rem] flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-xl px-4 py-2 text-sm">
        ...
        <div className="mt-7 flex w-full max-w-sm flex-col items-center">
          <div className="mb-5 flex flex-wrap items-center justify-center gap-2">
            <div className="rounded-xl border border-[rgba(var(--line),0.45)] bg-[rgba(var(--bg),0.28)] px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">
              {activeProject ? `${activeProject.title}${activeTaskName ? ` › ${activeTaskName}` : ""}` : "Focus block"}
            </div>
            {activeTodoItemTitle ? (
              <div className="rounded-xl border border-[rgba(var(--line),0.28)] bg-[rgba(var(--bg),0.18)] px-3.5 py-1.5 text-[10px] font-medium uppercase tracking-[0.16em] text-[rgb(var(--muted))]">
          ...
          <div className="grid w-full grid-cols-[1fr_auto] gap-3">
            <button
              ...
              className={cn(
                "flex items-center justify-center gap-3 rounded-xl px-5 py-4 text-lg font-semibold transition-all active:scale-[0.99]",
                timer.isRunning 
            ...
            <button
              onClick={handleReset}
              className="rounded-xl border border-[rgba(var(--line),0.45)] bg-[rgba(var(--bg),0.24)] p-4 text-white transition-all hover:bg-white/10"
              title="Reset"
            >
